...
\begin{enumerate}[label=\textbf{C\arabic*.},leftmargin=2.2em]
  ...
  \begin{itemize}[leftmargin=1.2em,itemsep=2pt,topsep=1pt]
    ...
    \begin{itemize}[leftmargin=1.2em,itemsep=1pt]
      ...
      \item \emph{Unfair Comparison:} Cerqueira et al.~\cite{cerqueira2026framework} emphasize that heuristic parameter choices or tuning on test data creates severe evaluation bias.
      \item \emph{Core Objective:} Identify hyperparameter configurations that perform robustly and objectively across diverse datasets, rather than finding an isolated "best" parameter for a single dataset.
    \end{itemize}

    \item \textbf{Reference Baseline - Leave-One-Dataset-Out (LODO):}
    \begin{itemize}[leftmargin=1.2em,itemsep=1pt]
      \item \emph{Mechanism (Cerqueira et al.~\cite{cerqueira2026framework}):} Use $n-1$ datasets to search for optimal configurations (via random search) and evaluate on the remaining held-out dataset. Keeps tuning independent from test data.
    ...
    \begin{enumerate}[label=(\alph*),leftmargin=1.4em,itemsep=2pt]
      ...
      \end{itemize}
      \item \textbf{Core Benefit:} Supported by the precedent of Fan et al.~\cite{fan2020hyperparameter}, a single scaling coefficient $c$ automatically adapts to data density (larger $\epsilon$ for sparse data, smaller for dense data), eliminating the flaw of enforcing rigid absolute values across heterogeneous datasets.
    \end{enumerate}
  \end{itemize}
\end{enumerate}
