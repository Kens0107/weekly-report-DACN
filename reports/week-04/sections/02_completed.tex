...
\begin{enumerate}[label=\textbf{C\arabic*.},leftmargin=2.2em]
  ...
  \begin{itemize}[leftmargin=1.2em,itemsep=3pt,topsep=2pt]
    ...
    \item \textbf{P2R, LODO on a selection rule.} Every clustering algorithm has a granularity knob $g$, such as $k$ or $\varepsilon$, whose right value depends on the data. So instead of transferring a value of $g$, P2R transfers a rule for picking it. The shape parameters $\theta$ ($\beta,\mu$ or ratio, $t$) are chosen by LODO, and $g$ is picked again on each new dataset from a label-free curve:
    \[
